    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        if (attachment_data.startsWith('http')) {
          ...
          const text = Buffer.from(buffer).toString('utf-8', 0, 100);
          if (text.includes('\x00') || attachment_data.includes('.xlsx')) {
            csvText = await excelToCSV(Buffer.from(buffer));
          } else {
            csvText = Buffer.from(buffer).toString('utf-8');
          }
        }
        else if (attachment_data.includes(',')) {
          const [mimeType, base64String] = attachment_data.split(',');
          const buffer = Buffer.from(base64String, 'base64');
          if (mimeType.includes('spreadsheet') || mimeType.includes('excel')) {
            csvText = await excelToCSV(buffer);
          } else {
