...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const request_id = fields.request_id?.[0] || fields.request_id;
    const caf_number = fields.caf_number?.[0] || fields.caf_number;
    ...
    if (!htmlFile) {
      return res.status(400).json({
        error: 'Missing required file: html_file',
        details: 'Send file as multipart/form-data with field name "html_file"',
        timestamp: new Date().toISOString()
      });
    }
    ...
    try {
      ...
      if (!htmlContent || htmlContent.length < 100) {
        return res.status(400).json({
          error: 'Invalid HTML file - content too short or empty',
          timestamp: new Date().toISOString()
        });
      }
      ...
      const parser = new IRSEmploymentParser(htmlContent);
      ...
      const { data: updated, error: updateError } = await supabase
        ...
        .update({
          ...
          raw_transcript_html: htmlContent,
      ...
      return res.status(200).json({
        request_id,
        status: 'completed',
        message: 'Transcript file uploaded, parsed, and results stored',
        file_info: {
          filename: htmlFile.originalFilename || 'transcript.html',
          size_bytes: htmlFile.size,
